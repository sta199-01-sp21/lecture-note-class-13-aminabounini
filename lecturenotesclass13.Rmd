...
It is more likely to be inaccurate than an interval.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-aminabounini/manhattan.csv")
```
...
This gives us how confident it is that it contains the estimated value.
...
- Sample: mice that died while not treatment.
- Sample size: 522
...
We are 95% confident that the percentage of mice that died while not on the 
treatment is between 69% and 80%.
**Interpretation**:
...
This means that we are 90% confident that the percentage of mice that lived while
on treatment is between 35% and 45%.
